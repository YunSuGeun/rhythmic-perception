%% Generate curve-fitting figures from Fig5_Curvefitting_sensitivity.py CSV results
% Saves sensitivity M1-M4 figures as PNG and PDF.

fprintf('--- FIGS3 CURVE FIGURE GENERATION  START ---\n');
drawnow('update');
...
saveRoot = fullfile(projectRoot, '03_Figure', 'FigureS3');
...
fprintf('--- FIGS3 CURVE FIGURE GENERATION  COMPLETE ---\n');


function yhat = predict_fit(row, modelName, timeSec)
    freqHz = row.frequency_hz(1);
    % Mirrors Fig5_Curvefitting_sensitivity.py output: c is slope, k is intercept.
    ...
    if strcmp(modelName, 'M4')
        envelope = gaussian_damping(row.sigma(1), timeSec);
        yhat = envelope .* oscillation + trend;
        return;
    end
...
function y = gaussian_damping(sigma, timeSec)
    sigma = max(double(sigma), 1e-9);
    exponent = -(timeSec .^ 2) ./ (2 .* sigma .^ 2);
    y = exp(min(max(exponent, -60), 0));
end
